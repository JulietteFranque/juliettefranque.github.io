...
class one_dim_conduction(physical_models):
    def __init__(self, T_f, T_b, time, h_f, h_b, epsilon_DFT, model='one_d_conduction', Kelvin=False, L_i=0.019, plate_thickness=0.0016, plate_material=stainless_steel, insulation=ceramic_fiber, T_inf=None, T_sur=None, nodes=10, run_on_init=True, T_0=None, *args, **kwargs):
        """
        Parameters
        ----------
            T_f: array like
    ...
    def incident_heat_flux(self):
        ...
        self.q_rad = (sigma * self.eps * (self.T_f**4 - self.T_sur**4) + sigma * self.eps * (self.T_b**4 - self.T_sur**4))/1000

        self.q_inc = 1/self.eps*(self.q_net + self.q_rad + self.q_conv)
    ...
    def one_d_conduction(self):
        ...
        self.delta_x = self.L_i/(self.nodes+1)
        time_steps = len(self.time)
        self.T_nodes_i = np.ones((time_steps, self.nodes))*self.T_0

        self.delta_t = self.time[1] - self.time[0]

        # Insulation Fourrier number
        self.Fo_i = self.delta_t*self.insul.alpha/self.delta_x**2
       
        # Insulation implicit matrix construction and inversion
        self.A_i_inv = construct_banded_matrix(Fo=self.Fo_i, nodes=self.nodes)

        # Loop over time
        for t in range(time_steps-1):
            # Create temporary vector for current time step
            Temp_i = self.T_nodes_i[t].copy()
            # Update boundary condition at x=0
            Temp_i[0] = Temp_i[0] + self.Fo_i*self.T_f[t+1]
            # Update boundary condition at x=L
            Temp_i[-1] = Temp_i[-1] + self.Fo_i*self.T_b[t+1]
            # Solve for temperatures at next time step
            self.T_nodes_i[t+1] = self.A_i_inv @ Temp_i

        #q_{cond} = -k_i A_{i} \frac{dT_i}{dx}|_{x=0} (Watts)
        self.q_cond_if = -self.insul.k*(self.T_nodes_i[:,0] - self.T_f)/self.delta_x
        self.q_cond_ib = -self.insul.k*(self.T_nodes_i[:,-1] - self.T_b)/self.delta_x

        self.q_st_ins = (self.q_cond_if - self.q_cond_ib)/1000
       # self.q_st_ins = self.insul.rCp*self.delta_x*np.gradient(self.T_nodes_i, self.time,axis=0).sum(axis=1)/1000

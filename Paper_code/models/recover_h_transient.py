...
class RecoverHtransient:
    ...
    def recover_h_simple(self):
        ...
        for it in range(self.it_number):
            self.u_avg.append(np.mean(self.u))
            ...
            self.u = dft.Correlations(self.T_TC_trim, self.T_inf,
                                      self.D_TC).inverse_sphere_correlation(self.h_TC)

            self.h_f = dft.Correlations(self.T_DFT_front,
                                        T_eval,
    ...
    def h_energy_bal_TC(self):
        dTdt = np.gradient(self.T_TC_trim, self.t)
        self.h_TC = 1 / (self.A_TC * (self.T_TC_trim - self.T_inf)) * (
                -self.rho_TC * self.C_TC * self.V_TC * dTdt + self.eps_TC * self.A_TC *
                self.q_inc_iter - self.eps_TC * self.sig * self.A_TC * (
                    self.T_TC_trim ** 4 - self.T_sur ** 4))
